...
The remaining arguments will be left at their default values (see section \ref{sec:general-parameters} for details). For the \code{simplex()} function, this means that the embedding dimension, $E$, will range from $1$ to $10$.
...
<<simplex-e, fig = TRUE, include = FALSE, height = 3, width = 4.5>>=
...
<<simplex-tp, fig = TRUE, include = FALSE, height = 3, width = 4.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
<<smap-theta, fig = TRUE, include = FALSE, height = 3, width = 4.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
<<smap-theta-noisy, fig = TRUE, include = FALSE, height = 3, width = 4.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
output <- block_lnlp(block_3sp, lib = lib, pred = pred,
...
<<>>=
output_2 <- block_lnlp(block_3sp, lib = lib, pred = pred,
...
# test for equality
stopifnot(identical(output, output_2))
@
...
<<>>=
list_of_model_predictions <- output$model_output
...
<<block-lnlp-obs-vs-pred, fig = TRUE, include = FALSE, height = 3.5, width = 3.5>>=
par(pty = "s", mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
<<>>=
smap_coeffs <- output$smap_coefficients[[1]]
str(smap_coeffs)
@
...
<<smap-coefficients, fig = TRUE, include = FALSE, height = 6, width = 5>>=
par(mfrow = c(4, 1), mar = c(2, 4, 1, 1), mgp = c(2.5, 1, 0))
...
plot(t, smap_coeffs[, 1], type = "l", col = "red", 
     ylab = "effect of x", xlab = "")
plot(t, smap_coeffs[, 2], type = "l", col = "blue", 
     ylab = "effect of y", xlab = "")
plot(t, smap_coeffs[, 3], type = "l", col = "magenta", 
...
output <- block_gp(block_3sp, lib = lib, pred = pred,
                   columns = cols, target_column = target,
                   stats_only = FALSE, first_column_time = TRUE, 
                   silent = TRUE)
...
<<gp-obs-vs-pred, fig = TRUE, include = FALSE, width = 3.5, height = 3.5>>=
observed <- output$model_output[[1]]$obs
predicted <- output$model_output[[1]]$pred
...
# multiple values for \code{k} can be provided, 
...
<<multiview, fig = TRUE, include = FALSE, width = 6, height = 6>>=
...
anch_xmap_sst <- ccm(sardine_anchovy_sst, E = 3,
...
sst_xmap_anch <- ccm(sardine_anchovy_sst, E = 3,
...
<<ccm-anchovy-sst, fig = TRUE, include = FALSE, height = 3, width = 4.5>>=
a_xmap_t_means <- ccm_means(anch_xmap_sst)
t_xmap_a_means <- ccm_means(sst_xmap_anch)

par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
plot(a_xmap_t_means$lib_size, pmax(0, a_xmap_t_means$rho), 
...
legend("topleft", legend = c("anchovy xmap SST", "SST xmap anchovy"),
...
library(ggplot2)
ccm_fig <- ggplot(output, aes(x = tp, y = rho, color = direction)) +
    geom_line() + theme_bw()
@

<<ccm-time-delays, fig = TRUE, include = FALSE, width = 5, height = 3.4>>=
print(ccm_fig)
@
...
<<>>=
data(e120_invnit16)
str(e120_invnit16, strict.width = "cut")
@
...
# rescale each time series within a plot
data_by_plot <- split(composite_ts, e120_invnit16$Plot)
normalized_data <- lapply(data_by_plot, function(df) sapply(df, rescale))
...
<<simplex-e120, fig = TRUE, include = FALSE, width = 6, height = 4>>=
...
<<smap-e120, fig = TRUE, include = FALSE, width = 6, height = 4>>=
...
<<>>=
AB_columns <- c("AbvBioAnnProd", "AbvBioAnnProd_1", 
                "AbvBioAnnProd_2", "AbvBioAnnProd_3")
AB_out <- block_lnlp(block_data, 
                     lib = composite_lib, pred = composite_pred, 
                     columns = AB_columns, 
                     target_column = "AbvBioAnnProd", 
                     stats_only = FALSE)

Prec_columns <- c(AB_columns, "SummerPrecip.mm.")
Prec_out <- block_lnlp(block_data,
                       lib = composite_lib, pred = composite_pred, 
...
<<obs-pred-e120, fig = TRUE, include = FALSE, width = 3.5, height = 3.5>>=
par(pty = "s", mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
observed_AB <- AB_out$model_output[[1]]$obs
predicted_AB <- AB_out$model_output[[1]]$pred

observed_Prec <- Prec_out$model_output[[1]]$obs
predicted_Prec <- Prec_out$model_output[[1]]$pred
...
points(observed_Prec, predicted_Prec, pch = 2, col = "red")
abline(lm(predicted_Prec ~ observed_Prec), col = "red", lty = 3, lwd = 2)

legend("bottom", legend = c(paste("(biomass alone) rho =", 
                                  round(AB_out$rho, 2)), 
...
There is a marked improvement in forecast skill when including precipitation as a predictor, and the relationship between predicted and observed values falls closer to the 1:1 line (compare the red dashed line to the black dashed line). However, there is still a great deal of variability in biomass that is unexplained ($R^2 = \Sexpr{round(Prec_out$rho^2, 2)}$).
...
<<ccm-e120-inv-no, fig = TRUE, include = FALSE, width = 4.5, height=3>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))

plot(inv_xmap_no_means$lib_size, pmax(0, inv_xmap_no_means$rho), 
     type = "l", col = "red", ylim = c(0, 0.15), lwd = 2, 
     xlab = "Library Size", ylab = "Cross Map Skill (rho)")
lines(no_xmap_inv_means$lib_size, pmax(0, no_xmap_inv_means$rho), 
      col = "blue", lwd = 2)
legend(x = "topleft", col = c("red", "blue"), lwd = 2, bty = "n", 
...
<<ccm-e120-inv-abv, fig = TRUE, include = FALSE, width = 4.5, height=3>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
plot(inv_xmap_abv_means$lib_size, pmax(0, inv_xmap_abv_means$rho), 
...
While there is an initial peak in forecast skill at \code{E = 4}, the maximum $\rho$ is at \code{E = 8}. This suggests that 4 and 8 are both practical embedding dimensions.
...
\item \code{lib}
  ...
  + e.g., \code{(1, n)} specifies that the first n *rows* (from 1 to n) of data are a contiguous time series block, each point of which can be used to construct state vectors
  + by default, \code{lib} uses the entire input as a single contiguous segment
\item \code{pred}
  + (same format as \code{lib}, but specifes the portions of the time series to make predictions for)
\item \code{norm_type}
  + \code{"L2 norm"} (default) or \code{"L1 norm"}: specifies which distance metric to use when doing calculations
  + \code{"L2 norm"} is the standard Euclidean distance, where the distance between a vector $\mathbf{x} = \left(x_1, x_2, \dots, x_n \right)$ and $\mathbf{y} = \left( y_1, y_2, \dots, y_n \right)$ is computed as $\sqrt{(x_1 - y_1)^2 + (x_2 - y_2)^2 + \dots + (x_n - y_n)^2}$.
  + \code{"L1 norm"} is the Manhattan norm (also known as taxicab distance), where the distance between a vector $\mathbf{x} = \left( x_1, x_2, \dots, x_n \right)$ and $\mathbf{y} = \left( y_1, y_2, \dots, y_n \right)$ is computed as $|x_1 - y_1| + |x_2 - y_2| + \dots + |x_n - y_n|$.
  + \code{"P norm"} is the generalization of the L1 and L2 norms which adds a parameter $P$ as the exponent: the distance between a vector $\mathbf{x} = \left( x_1, x_2, \dots, x_n \right)$ and $\mathbf{y} = \left(y_1, y_2, \dots, y_n \right)$ is computed as ${\left(\left|x_1 - y_1\right|^P + \left|x_2 - y_2\right|^P + \dots + \left|x_n - y_n\right|^P\right)^{1/P}}$.
\item \code{P}
...
\item \code{tau}
  ...
  + by default, \code{tau} is set to \code{1}
\item \code{tp}
  + the prediction horizon (how many steps ahead to make forecasts)
  + negative values will also work
  + by default, \code{tp} is set to \code{1} for forecasting functions, and \code{0} for cross mapping
\item \code{num_neighbors}
  + the number of neighbors to use
  + \code{"e+1"}, \code{"E+1"}, \code{"e + 1"}, and \code{"E + 1"} will all peg this parameter to be \code{E+1} for each run
  + values less than 1 will use all possible neighbors
\item \code{theta}
  + the nonlinear tuning parameter (for use with S-maps) that adjusts how distance is factored into computation of the local linear map (\code{0} corresponds to a globally linear map, while values greater than 0 correspond to nonlinear models where the local linear map changes as a function of state-space)
\item \code{stats_only}
  + \code{TRUE} (default) or \code{FALSE}: specifies whether the output should just contain statistics of the predictions, or also include all the predictions that were made
  + if enabled, adds a \code{model_output} list-column, which contains the time index, observed values, predictions, and variance of predictions for each model
\item \code{exclusion_radius}
  + \code{exclusion_radius} sets the threshold whereby all vectors with time indices too close to the predictee will be excluded from being considered nearest neighbors
  + e.g., \code{1} means that vectors must have an associated \emph{time index} more than 1 away from potential nearest neighbors
  + by default, \code{exclusion_radius} is set to \code{NULL} (turning this filtering off)
\item \code{epsilon}
  + \code{epsilon} sets the threshold whereby all vectors with distance too far away from the predictee will be excluded from being considered nearest neighbors
  + e.g., \code{2} means that vectors must have be within a \emph{distance} of 2 from potential nearest neighbors
  + by default, \code{epsilon} is set to \code{NULL} (turning this filtering off)
\item \code{silent}
  + \code{TRUE} or \code{FALSE} (default): specifies whether to suppress warning messages from being printed to the R console
\item \code{save_smap_coefficients}
  + \code{TRUE} or \code{FALSE} (default): specifies whether to include a data.frame of S-map coefficients with the output
  + (note that setting this to \code{TRUE} forces the full output as if \code{stats_only = FALSE})
  + if enabled, adds a \code{smap_coefficients} list-column, which contains the s-map coefficients; and a \code{smap_coefficient_covariance} list-column, which contains the covariance matrices for the s-map coefficients.
\end{itemize}
...
\item \code{fit_params}
  + \code{TRUE} (default) or \code{FALSE}: specifies whether to fit the above parameters over the \code{lib} portion of the data, or use the parameters as given
\item \code{save_covariance_matrix}
  + \code{TRUE} or \code{FALSE} (default): specifies whether to include a matrix for the covariance between predictions
\end{itemize}
