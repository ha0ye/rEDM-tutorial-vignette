...
<<echo=FALSE>>=
options(digits = 2, width = 75, str = list("strict.width" = "cut", 
                                           "digits.d" = 3, 
                                           "vec.len" = 4))
@
...
output <- multiview(block, lib = lib, pred = pred,
...
<<ccm-time-delays, fig = TRUE, include = FALSE, width = 5, height = 3.5>>=
print(ccm_fig)
@
...
AB_out <- block_lnlp(block_data, target_column = "AbvBioAnnProd", 
                     lib = composite_lib, pred = composite_pred, 
                     columns = AB_columns, stats_only = FALSE)

Prec_columns <- c(AB_columns, "SummerPrecip.mm.")
Prec_out <- block_lnlp(block_data, target_column = "AbvBioAnnProd", 
                       lib = composite_lib, pred = composite_pred, 
...
<<thrips-time-series, fig = TRUE, include = FALSE, width = 5, height = 6>>=
par(mfrow = c(4, 1), mar = c(2, 4, 1, 1), oma = c(2, 0, 0, 0),
    mgp = c(2.5, 1, 0))
t <- as.Date(paste0(thrips_block$Year, "-", thrips_block$Month, "-15"))
plot(t, thrips_block$Thrips_imaginis, 
     type = "l", col = "green", ylab = "Thrips", xlab = "")
plot(t, thrips_block$maxT_degC, type = "l", 
     col = "red", ylab = "maxT (oC)", xlab = "")
plot(t, thrips_block$Rain_mm, type = "l", 
     col = "blue", ylab = "Rain (mm)", xlab = "")
plot(t, thrips_block$Season, type = "l", 
...
<<thrips-simplex, fig = TRUE, include = FALSE, width = 5, height = 3.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
<<thrips-smap, fig = TRUE, include = FALSE, width = 6, height = 3.5>>=
...
plot(smap_output[[1]]$theta, smap_output[[1]]$rho, 
     type = "l", xlim = c(0, 4),
     xlab = "Nonlinearity (theta)", ylab = "Forecast Skill (rho)")
plot(smap_output[[2]]$theta, smap_output[[2]]$rho, 
     type = "l", xlim = c(0, 4),
     xlab = "Nonlinearity (theta)", ylab = "Forecast Skill (rho)")
@
...
ccm_out <- list(ccm_means(thrips_xmap_maxT), 
                ccm_means(maxT_xmap_thrips))
@

<<thrips-temp-ccm, fig = TRUE, include = FALSE, width = 5, height = 3.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
plot(ccm_out[[1]]$lib_size, pmax(0, ccm_out[[1]]$rho), 
     type = "l", col = "red", ylim = c(0, 1), 
     xlab = "Library Size", ylab = "Cross Map Skill (rho)")
lines(ccm_out[[2]]$lib_size, pmax(0, ccm_out[[2]]$rho), col = "blue")
abline(h = corr_matrix["Thrips_imaginis", "maxT_degC"], 
       col = "black", lty = 2)
legend("bottomright", legend = c("Thrips xmap maxT", "maxT xmap Thrips"),
       col = c("red", "blue"), lwd = 1, bty = "n", cex = 0.8)
@
...
<<>>=
thrips_xmap_Rain <- ccm(thrips_block, lib_sizes = seq(10, 75, by = 5), 
                        E = 8, random_libs = TRUE, num_samples = 300, 
                        lib_column = "Thrips_imaginis", 
                        target_column = "Rain_mm", silent = TRUE)
Rain_xmap_thrips <- ccm(thrips_block, lib_sizes = seq(10, 75, by = 5), 
                        E = 8, random_libs = TRUE, num_samples = 300, 
                        lib_column = "Rain_mm", 
...
<<thrips-rain-ccm, fig = TRUE, include = FALSE, width = 5, height = 3.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
plot(ccm_out[[1]]$lib_size, pmax(0, ccm_out[[1]]$rho), 
     ...
     xlab = "Library Size", ylab = "Cross Map Skill (rho)")
lines(ccm_out[[2]]$lib_size, pmax(0, ccm_out[[2]]$rho), col = "blue")
abline(h = corr_matrix["Thrips_imaginis", "Rain_mm"], 
       col = "black", lty = 2)
legend("topleft", col = c("red", "blue"), bty = "n", cex = 0.8, 
...
ccm_out <- list(ccm_means(thrips_xmap_Season), 
                ccm_means(Season_xmap_thrips))
@

<<thrips-season-ccm, fig = TRUE, include = FALSE, width = 5, height = 3.5>>=
par(mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
...
plot(ccm_out[[1]]$lib_size, pmax(0, ccm_out[[1]]$rho), 
     ...
     xlab = "Library Size", ylab = "Cross Map Skill (rho)")
lines(ccm_out[[2]]$lib_size, pmax(0, ccm_out[[2]]$rho), col = "blue")
abline(h = corr_matrix["Thrips_imaginis", "Season"], 
       col = "black", lty = 2)
legend("bottomright", col = c("red", "blue"), bty = "n", cex = 0.8, 
...
<<>>=
num_surr <- 1000
surr_maxT <- make_surrogate_data(thrips_block$maxT_degC, 
                                 method = "seasonal",
                                 T_period = 12, num_surr = num_surr)
surr_Rain <- make_surrogate_data(thrips_block$Rain_mm, 
                                 method = "seasonal",
                                 T_period = 12, num_surr = num_surr)

ccm_rho_surr <- data.frame(maxT = numeric(num_surr), 
...
<<>>=
ccm_rho_rain <- ccm_rho_matrix["Thrips_imaginis", "Rain_mm"]
(sum(ccm_rho_rain < ccm_rho_surr$Rain) + 1) /
    (length(ccm_rho_surr$Rain) + 1)
@

<<>>=
ccm_rho_maxT <- ccm_rho_matrix["Thrips_imaginis", "maxT_degC"]
(sum(ccm_rho_maxT < ccm_rho_surr$maxT) + 1) /
